    ...
    public synchronized void sendMessageToClient(final String toNickName, final ClientHandler from,
        ...
        for (ClientHandler client : clients) {
            if (client.getNickName().equals(toNickName)) {
                from.sendMsg(from.getCurrentDate() + ChatConstant.PRIVATE_TO + client.getNickName() + ": " + message);
                client.sendMsg(from.getCurrentDate() + ChatConstant.PRIVATE_FROM + from.getNickName() + ": " + message);
                return;
            }
        }
        from.sendMsg(ChatConstant.NO_SUCH_USER_IN_ROOM);
    }
