...
import ru.shafikov.javacore.ChatConstant;
...
public final class ClientHandler {
    ...
    private void handleClientRequest() throws IOException {
        while (true) {
            ...
            if (message.startsWith(ChatConstant.AUTHORIZE_FLAG)) {
                if (authorizeClient(message)) break;
            } else if (message.startsWith(ChatConstant.REGISTER_FLAG)) {
    ...
    private boolean authorizeClient(final String message) {
        ...
        if (nickName != null) {
            if (!server.isNickNameBusy(nickName)) {
                sendMsg(ChatConstant.AUTHORIZE_OK_FLAG + nickName);
                this.nickName = nickName;
                server.broadcastMsg(this.nickName + ChatConstant.CHAT_ENTERED_FLAG);
        ...
        if (server.getAuthService() instanceof BaseAuthService) {
            ...
            if (!containsLogin) {
                if (!containsNickName) {
                    ...
                    sendMsg(ChatConstant.REGISTER_OK_FLAG + nickName);
                    this.nickName = nickName;
                    server.broadcastMsg(this.nickName + ChatConstant.CHAT_ENTERED_FLAG);
                    server.subscribe(this);
                    return true;
    ...
    private void getMessages() throws IOException{
        while (true) {
            ...
            System.out.println(ChatConstant.FROM + nickName + ": " + messageText);
            if (ChatConstant.TERMINATE_CONNECTION_FLAG.equals(messageText)) break;

            if (messageText.startsWith(ChatConstant.PRIVATE_MESSAGE_FLAG)) {
                final String[] parts = messageText.split("\\s");
                final String nickName = parts[1];
                final String message = messageText.substring(ChatConstant.PRIVATE_MESSAGE_FLAG.length()
                        + nickName.length() + 1);
